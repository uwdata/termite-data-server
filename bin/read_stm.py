...
def ImportSTM( app_name, model_path, corpus_path, database_path, is_quiet, force_overwrite ):
	...
	logger.info( '--------------------------------------------------------------------------------' )
	logger.info( 'Import a STM topic model as a web2py application...' )
	...
	if force_overwrite or not os.path.exists( app_path ):
		with CreateApp( app_name ) as app:
			with LDA_DB( app.GetDatabasePath() ) as lda_db:
				reader = STMReader( model_path, lda_db )
				reader.Execute()
...
def main():
	...
	parser.add_argument( 'model_path'   , type = str                     , help = 'Output of a STM topic model' )
