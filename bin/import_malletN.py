...
from importers.MalletLDAs import MalletLDAs

def ImportMalletLDA( app_name, model_path, corpus_path, meta_path, num_models, is_quiet, force_overwrite ):
	...
	if force_overwrite or not os.path.exists( app_path ):
		with CreateApp( app_name ) as createApp:
			importMalletLDAs = MalletLDAs( createApp.GetPath(), model_path, num_models )
			if force_overwrite or not importMalletLDAs.Exists():
				importMalletLDAs.Execute()
...
def main():
	...
	ImportMalletLDA( args.app_name, args.model_path, args.corpus_path, args.meta_path, args.models, args.quiet, args.overwrite )
